...
public sealed class ProjectStoreTests
{
    ...
    public async Task EnsureProjectAsync_NewProject_CreatesIt()
    {
        ...
        var project = await _sut.EnsureProjectAsync(name, new EmbeddingModel("bge-m3", 1024));
    ...
    public async Task EnsureProjectAsync_SameNameTwice_ReturnsTheSameProjectId()
    {
        ...
        var first = await _sut.EnsureProjectAsync(name, new EmbeddingModel("bge-m3", 1024));
        var second = await _sut.EnsureProjectAsync(name, new EmbeddingModel("bge-m3", 1024));
    ...
    {
        var name = TestData.NewProjectName();
        await _sut.EnsureProjectAsync(name, new EmbeddingModel("bge-m3", 1024));

        var updated = await _sut.EnsureProjectAsync(name, new EmbeddingModel("nomic-embed-text", 768));
