...
/// timestamped migrations. Every table's actual primary key is a numeric, auto-incrementing
/// <c>id</c>, never returned by the API; <c>projects</c> and <c>indexing_runs</c> each also carry a
/// <c>public_id</c> (UUIDv7, generated by the application) that every port, controller and response
/// uses instead.
/// </summary>
...
public sealed class InitialSchema : Migration
{
    ...
    private void CreateProjectsTable()
    {
        Create.Table("projects")
            ...
            .WithColumn("git_url").AsString(int.MaxValue).Nullable()
            .WithColumn("git_raw_url").AsString(int.MaxValue).Nullable()
        ...
        Create.UniqueConstraint("ux_projects_public_id").OnTable("projects").Columns("public_id");
    }

    private void CreateIndexingRunsTable()
    {
        // "id" is the real primary key: numeric, auto-incrementing, never exposed by the API.
        // "public_id" is the identifier every port, controller and response actually works with,
        // and what the indexing_run_id column on ciir_uploads (below) references.
        ...
        Create.UniqueConstraint("ux_indexing_runs_public_id").OnTable("indexing_runs").Columns("public_id");
